...
//
//---------------------------------------------------------------------------------
namespace devMobile.TheThingsNetwork.AzureDeviceClientSetReceiveMessageHandlerSingle
{
   using System;
   using System.IO;
   using System.Text;
...
#if RECEIVE_HANDLER
            await azureIoTHubClient.SetReceiveMessageHandlerAsync(ReceiveMessageCallback, azureIoTHubClient);
#endif
      ...
      public static void TimerCallbackSync(object state)
      {
         ...
         try
         {
            ...
            using (Message message = new Message(Encoding.ASCII.GetBytes(JsonConvert.SerializeObject(payload))))
            {
               ...
               azureIoTHubClient.SendEventAsync(message);
...
#if RECEIVE_HANDLER
      public static async Task ReceiveMessageCallback(Message message, object status)
      {
         DeviceClient deviceClient = (DeviceClient)status;

         Console.WriteLine();
         Console.WriteLine("ReceiveMessageCallback");
         Console.WriteLine($"MessageID:{message.MessageId}");
         Console.WriteLine($"ComponentName:{message.ComponentName}");
         Console.WriteLine($"ConnectionDeviceId:{message.ConnectionDeviceId}");
         Console.WriteLine($"ConnectionModuleId:{message.ConnectionModuleId}");
